...
def lumpy_input(lumpy_file):
    ...
    for i in range(len(lumpy_sum) - 1):
        ...
        if lumpy_sum[i][7].split(";")[0] == "SVTYPE=BND" and lumpy_sum[i][2].split("_")[1] == "1" and lumpy_sum[i + 1][2].split("_")[1] == "2" and \
                        lumpy_sum[i][2].split("_")[0] == lumpy_sum[i + 1][2].split("_")[0] and lumpy_sum[i][0] == lumpy_sum[i + 1][0] and abs(int(str(lumpy_sum[i][1]))-int(str(lumpy_sum[i+1][1]))) > 10000 and str(lumpy_sum[i][0]) != "NC_001416.1":
                    lumpy_intra.append(lumpy_sum[i] + lumpy_sum[i + 1])
        elif lumpy_sum[i][7].split(";")[0] != "SVTYPE=BND" and abs(int(str(lumpy_sum[i][7].split(";")[2].split("=")[1]))) > 10000:
    ...
    for j in range(len(common_wt)):
        for i in range(len(common_event)):
            if common_event[i][0] == common_wt[j][0] and (
                        int(common_wt[j][1]) - 10000) < int(common_event[i][1]) < (
                        int(common_wt[j][1]) + 10000) and (
                        int(common_wt[j][11]) - 10000) < int(common_event[i][11]) < (int(common_wt[j][11]) + 10000) and \
                            common_event[i] not in common:
                common.append(common_event[i])
            elif common_event[i][0] == common_wt[j][0] and (
                        int(common_wt[j][11]) - 10000) < int(common_event[i][1]) < (
                        int(common_wt[j][11]) + 10000) and (
                        int(common_wt[j][1]) - 10000) < int(common_event[i][11]) < (int(common_wt[j][1]) + 10000) and \
                            common_event[i] not in common:
                common.append(common_event[i])
...
def output(common, file):
    with open(file, "w") as circos:
        for i in range(len(common)):
            if 3 < abs(int(str(common[i][7].split(";")[1].split(":")[1].split(",")[0]))) < 20:
                if common[i][7].split(";")[0] == "SVTYPE=BND": 
                    circos.write(str(int(i + 1)) + "\t" + "os" + common[i][0] + "\t" + common[i][1] + "\t" + str(int(common[i][1]) + 1) + "\n" + str(int(i + 1)) + "\t" + "os" + common[i][10] + "\t" +common[i][11] + "\t" + str(int(common[i][11]) + 1) + "\n")
                else: 
                    circos.write(str(int(i + 1)) + "\t" + "os" + common[i][0] + "\t" + common[i][1] + "\t" + str(int(common[i][1]) + 1) + "\t" + str(common[i][7].split(";")[0].split("=")[1]) + "\n" + str(int(i + 1)) + "\t" + "os" + common[i][10] + "\t" +common[i][11] + "\t" + str(int(common[i][11]) + 1) + "\t" + str(common[i][7].split(";")[0].split("=")[1]) + "\n")
