...
def lumpy_input(lumpy_file):
    ...
    for i in range(len(lumpy_sum) - 1):
        if lumpy_sum[i][7].split(";")[0] == "SVTYPE=BND" and lumpy_sum[i][0] != lumpy_sum[i + 1][0] and \
            ...
            if lumpy_sum[i][0] != "NC_001416.1" and lumpy_sum[i][0] != "pBAR184" and lumpy_sum[i + 1][
                0] != "NC_001416.1" and lumpy_sum[i + 1][0] != "pBAR184":
    ...
    for i in range(len(sample_inter)):
        for j in range(len(wt_inter)):
            if sample_inter[i][0] == wt_inter[j][0] and sample_inter[i][10] == wt_inter[j][10] and (
                        int(sample_inter[i][1]) - 1000000) < int(wt_inter[j][1]) < (int(sample_inter[i][1]) + 1000000) and (
                        int(sample_inter[i][11]) - 1000000) < int(wt_inter[j][11]) < (int(sample_inter[i][11]) + 1000000):
                common.append(sample_inter[i])
            elif sample_inter[i][0] == wt_inter[j][10] and sample_inter[i][10] == wt_inter[j][0] and (
                        int(sample_inter[i][11]) - 1000000) < int(wt_inter[j][1]) < (int(sample_inter[i][11]) + 1000000) and (
                        int(sample_inter[i][1]) - 1000000) < int(wt_inter[j][11]) < (int(sample_inter[i][1]) + 1000000):
                common.append(sample_inter[i])
...
def main():
    ...
    with open(circos_file, "w") as circos:
        for i in range(len(common_sum)):
            if 2 < abs(int(str(common_sum[i][7].split(";")[1].split(":")[1].split(",")[0]))) < 10: 
                circos.write(str(int(i + 1)) + "\t" + "zm" + common_sum[i][0] + "\t" + common_sum[i][1] + "\t" + str(int(common_sum[i][1]) + 1) + "\n" + str(int(i + 1)) + "\t" + "zm" + common_sum[i][10] + "\t" +common_sum[i][11] + "\t" + str(int(common_sum[i][11]) + 1) + "\n")
